...
const WalletService = {
  ...
  getDashboardInfo: async(username) => {
    ...
    let latestTransactions = await st.getLastNTransactions(profile.walletAddress, 5);
    // retrieve profiles
    let addresses = [];
    let transactions = [];
    if (latestTransactions.length > 0) {
      ...
      for (let i = 0; i < latestTransactions.length; i++) {
        ...
        transactions.push({
          ...
          wallet_address_prefix: (sent === true? latestTransactions[i].to.substring(0, 15):
            latestTransactions[i].from.substring(0, 15)),
  ...
  findProfiles: async(keywords) => {
    return await UserService.searchProfiles(keywords);
