...
BotSchema.post('save', async function(doc) {
  ...
  dotbot.ownerName = doc.user.username;
...
BotSubscriptionSchema.post('save', async function(doc) {
  await DotbotPublisher.deleteOne({botId: doc.bot._id, publisherName: doc.user.username});
  let dotsub = new DotbotPublisher({subscriptionId: doc._id, botId: doc.bot._id, publisherName: doc.user.username});
  ...
  if (bot.services.length > 0) {
    for (let j = 0; j < bot.services.length; j += 1) {
      ...
      service.ownerName = component.user.username;
