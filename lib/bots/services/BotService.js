...
const BotService = {
  ...
  validateBotComponents: async (bot) => {
    if (typeof bot.botEngine === 'undefined' ||
      typeof bot.botEngine.component === 'undefined') {
  ...
  createBot: async (data) => {
    ...
    await BotService.validateBotComponents(bot);
  ...
  updateSubscriptionsCount: async(botId) => {
  ...
  findSubscription: async (botId, userId) => {
    ...
    let subscription = {
      ...
      bot: {
        ...
        reviewsCount: bot.reviewsCount,
        subscriptionsCount: bot.subscriptionsCount,
        properties: [],
        services: [],
        channels: [],
        botengine: [],
  ...
  unsubscribe: async (botId, userId) => {
    ...
    await BotService.updateSubscriptionsCount(botId);
    return result;
  },
  ...
  subscribe: async (botId, userId, subscriptionType, properties,
    ...
    if (botengine instanceof Array) {
      if (botengine.length > 0) {
  ...
  findPaginatedBots: async (resultsPerPage, currentPage, userId, search, status,
    ...
    if (sortBy !== 'name' && sortBy !== 'subscriptionsCount' && 'averageRating'
    && sortBy !== 'updatedAt') {
      sortBy = '';
    }
